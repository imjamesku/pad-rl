...
class Agent01:
    def __init__(self,
        ...
        if self.dense_layers[-1] != self.tg_shape[0]:
            raise Exception(
                'ERROR: The last dense layer needs to have the shape of the target!')

        # Initialize models A and B.
        self.state_A, self.target_A, self.q_values_A, self.loss_A = self.initialize_model(
            scope='model_A')
        self.state_B, self.target_B, self.q_values_B, self.loss_B = self.initialize_model(
            scope='model_B')
        ...
        gradients, variables = zip(
            *self.optimizer.compute_gradients(self.loss_A))
        self.gradients = zip(gradients, variables)
        self.backprop_operation = self.optimizer.apply_gradients(
            self.gradients)
    ...
    def initialize_model(self, scope):
        with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
            ...
            state = tf.placeholder(
                tf.float32, shape=[None] + list(self.st_shape), name='state')
            target = tf.placeholder(
                tf.float32, shape=[None] + list(self.tg_shape), name='target')
            ...
            for i in range(len(self.conv_layers)):
                conv_layer_out = tf.layers.conv2d(
                    ...
                    kernel_size=(
                        self.conv_layers[i][0], self.conv_layers[i][0]),
                    strides=(1, 1),
                    padding='valid',
            ...
            relu_conv_layer = tf.nn.leaky_relu(
                conv_layer_out, name='model_core/leaky_relu')

            dense_layer_in = tf.contrib.layers.flatten(
                relu_conv_layer, scope='model_core/conv_flatten')
            ...
            q_values = tf.nn.relu(
                dense_layer_out, name='model_core/final_relu')
    ...
    def predict_from_state(self, batch_state, model='A'):
        ...
        q_value_predictions = self.sess.run(
            fetches=[q_values_tensor], feed_dict=inputs)[0]
    ...
    def copy_A_to_B(self, verbose=False):
        ...
        all_nodes = [node.values()
                     for node in tf.get_default_graph().get_operations()]
        all_names = [
            tensor.name for tensors in all_nodes for tensor in tensors]
        all_weight_names = []
        ...
        A_names = [
            name for name in all_weight_names if name.find('model_A') > -1]
        B_names = [
            name for name in all_weight_names if name.find('model_B') > -1]
        ...
        A_tensors = [tf.get_default_graph().get_tensor_by_name(name)
                     for name in A_names]
        B_tensors = [tf.get_default_graph().get_tensor_by_name(name)
                     for name in B_names]
        assign_ops = [tf.assign(B_tensor, A_tensor)
    ...
    def board_finger_to_state(self, boards, fingers):
        ...
        states = np.zeros(
            (batch_size, self.st_shape[0], self.st_shape[1], self.st_shape[2]))
    ...
    def tensorboard(self, data_path=None):
        ...
        print(
            'Type "tensorboard --logdir={0}" in your shell to view your model graph.'.format(data_path))
