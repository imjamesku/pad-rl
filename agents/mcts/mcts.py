...
def update_tree(root: TreeNode, board: NDArray[np.int8], prev_action: Literal['left', 'right', 'up', 'down', 'pass'], finger_position: tuple[int, int], min_steps: int, C: int):
    # not visited yet
    if root.hits == 0:
        value, _ = run_simulation(
            board, finger_position, prev_action, min_steps=0)
    ...
    # visited, so update children and back propagate
        ...
        # move finger
        next_finger_position, is_valid_action = move_finger(
            finger_position, max_action)
        assert is_valid_action, "Should always be valid"
        # swap orbs
        swap(board, finger_position, next_finger_position)
        # update children
        value = update_tree(
            root.children[max_action], board, max_action, next_finger_position, min_steps, C=C)
        # move finger back
        swap(board, finger_position, next_finger_position)
        # update hits & value
        root.hits += 1
...
def run_simulation(board: NDArray[np.int8], finger_position: tuple[int, int], prev_action: Literal['up', 'down', 'left', 'right'], min_steps: int):
    ...
    while True:
        ...
        cur_action = random.choice(get_valid_actions(
            cur_finger_position, prev_action, rows, cols, include_pass=can_pass))
        path.append(cur_action)
        if cur_action == 'pass':
            break
        next_finger_position, is_valid_action = move_finger(
            cur_finger_position, cur_action)
